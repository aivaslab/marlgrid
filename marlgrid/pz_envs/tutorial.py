...
class para_TutorialEnv(para_MultiGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        if level == 1: #empty, cluttered, and mazes
                    ...
                    for y in range(2, height-1, 2):
                        if sublevel in [3,4]:
                            ...
                            if num == 0:
                                ...
                                self.put_obj(Door(color=colors[0], state=randrange(0,3)), x, y)
                            elif num == 2:
                                self.put_obj(Key(color=colors[0], state=randrange(0,3)), x, y)
            if sublevel == 4:
                for i in range(3):
                    num = randrange(0,3)
                    if num == 0:
                        ...
                        self.place_obj(Door(color=colors[0], state=randrange(0,3)), top=(0, 0), size=(width, height))
                    elif num == 2:
                        self.place_obj(Key(color=colors[0], state=randrange(0,3)), top=(0, 0), size=(width, height))
            ...
            if sublevel in [3,4]:
                self.put_obj(Key(colors[0]), splitIdx-1, doorIdx)
            if sublevel in [5,6]:
                self.place_obj(Key(colors[0]), top=(1, 1), size=(splitIdx, height-1))
            if sublevel == 6:
                self.place_obj(Key(colors[1]), top=(1, 1), size=(splitIdx, height-1))
        ...
        elif level == 3: #preferential doorkey variants
            ...
            for color, goal, goal_p, door, sub in zip(colors, goals, goal_positions, door_positions, subgoal_positions):
                ...
                if sublevel == 1:
                    self.put_obj(Door(color=color, state=1), door[0], door[1])
                elif sublevel == 2:
